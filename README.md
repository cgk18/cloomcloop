...
- **Actions are ctrl+key** (`ctrl-b` branch, `ctrl-n` new, `ctrl-x` close, `ctrl-q` quit — destructive ones confirm with a second press), so stray typing in the sidebar can't fire anything. Navigation stays plain (arrows, enter, wheel).
